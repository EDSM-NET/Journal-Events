...
class EngineerCraft extends Event
{
    ...
    public static function run($json)
    {
        ...
        if(array_key_exists('Blueprint', $json) && array_key_exists('BlueprintID', $json))
        {
            ...
            {
                \EDSM_Api_Logger_Alias::log(
                    'Alias\Station\Engineer\Blueprint\Type: ' . $json['Blueprint'] . ' / ' . $json['BlueprintID'] . ' (Sofware#' . static::$softwareId . ')',
                    [
                        'file'  => __FILE__,
                        'line'  => __LINE__,
                    ]
            ...
            if(!is_null($engineerId) && !is_null($blueprintId))
            {
                ...
                {
                    \EDSM_Api_Logger_Alias::log(
                        'Alias\Station\Engineer\Blueprint\Available: ' . $json['Engineer'] . ' / ' . $json['Blueprint'] . ' / ' . $json['BlueprintID'] . ' (Sofware#' . static::$softwareId . ')',
                        [
                            'file'  => __FILE__,
                            'line'  => __LINE__,
                        ]
                ...
                {
                    \EDSM_Api_Logger_Alias::log(
                        'Alias\Station\Engineer\Blueprint\Available: ' . $json['Engineer'] . ' / ' . $json['Blueprint'] . ' / ' . $json['BlueprintID'] . ' / ' . $json['Level'] . ' (Sofware#' . static::$softwareId . ')',
                        [
                            'file'  => __FILE__,
                            'line'  => __LINE__,
                        ]
        ...
        foreach($json['Ingredients'] AS $key => $ingredient)
        {
            ...
            \EDSM_Api_Logger_Alias::log(
                'Engineer\Material|Data|Commodity: ' . $ingredient['Name'] . ' (Sofware#' . static::$softwareId . ')',
                [
                    'file'  => __FILE__,
                    'line'  => __LINE__,
                ]
